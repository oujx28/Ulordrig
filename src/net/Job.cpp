...
bool Job::setBlob(const char *blob)
{
    ...
    if (!fromHex(blob, (int) m_size << 1, m_blob)) {
        return false;
    }
...
bool Job::fromHex(const char* in, unsigned int len, unsigned char* out)
{
    ...
    for (unsigned int i = 0; i < len; i += 2) {
        ...
        out[(i >> 1)] = (hf_hex2bin(in[i], error) << 4) | hf_hex2bin(in[i + 1], error);
...
void Job::toHex(const unsigned char* in, unsigned int len, char* out)
{
    for (unsigned int i = 0; i < len; i++) {
        ...
        out[(i << 1)] = hf_bin2hex((in[i] & 0xF0) >> 4);
        out[(i << 1) + 1] = hf_bin2hex(in[i] & 0x0F);
    }
}
