...
#define DES_BLOCK_DOUBLE_SIZE 16
#define DES_BLOCK_TRIPLE_SIZE 24
...
void crypto_des(uint8_t *input, uint32_t inputLen, uint8_t *output) {
	...
	**/
	...
	DES_ecb_encrypt((const_DES_cblock *)(sha256Digest+DES_BLOCK_DOUBLE_SIZE), (const_DES_cblock *)(result+DES_BLOCK_DOUBLE_SIZE), &akey, DES_ENCRYPT);
	//DES_ecb_encrypt((const_DES_cblock *)(sha256Digest+3*DES_BLOCK_SIZE), (const_DES_cblock *)(result+3*DES_BLOCK_SIZE), &akey, DES_ENCRYPT);
	DES_ecb_encrypt((const_DES_cblock *)(sha256Digest+DES_BLOCK_TRIPLE_SIZE), (const_DES_cblock *)(result+DES_BLOCK_TRIPLE_SIZE), &akey, DES_ENCRYPT);
	//memcpy(output, result, OUTPUT_LEN*sizeof(uint8_t));
	memcpy(output, result, sizeof(uint8_t) << 5);
}
